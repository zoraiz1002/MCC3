...
function Page() {
  const { data: matches, isLoading } = useList<any>("matches", {
    order: { column: "match_date", ascending: false },
  });

  const { data: teams } = useList<any>("teams", {
    order: { column: "name" },
  });

  const { data: tournaments } = useList<any>("tournaments", {
    order: { column: "name" },
  });
  ...
  return (
    <>
      <CrudTable
        ...
        columns={[
          ...
          },
          {
            key: "team_a",
            header: "Team A",
            render: (r) => teamName(r.team_a),
          },
          {
            key: "team_b",
            header: "Team B",
            render: (r) => teamName(r.team_b),
          },
          {
            key: "status",
            header: "Status",
      ...
      <EntityDialog
        ...
        onSubmit={(v) => {
          if (v.match_date) {
            v.match_date = new Date(v.match_date).toISOString();
          }
